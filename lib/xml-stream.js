...
function XmlStream(stream, options) {
  ...
  this._encoding = opt.encoding || null;
...
function emitOneElement(element, name, onLeave) {
  if (typeof element === 'object') {
    ...
    if (__own.call(element, '$children')) {
      ...
      if (hasText) {
        emitText.call(this, element[`${opt.textNodeName}`]);
      }
    }
  } else {
    emitStart.call(this, name, element.$);
...
function parse() {
  ...
  xml.on('endElement', function(name) {    
    ...
    self._name = name;
    delete element.$;
    delete element[`${opt.textNodeName}`];
    delete element.$name;
    var val = element;
    ...
    if (text !== '') {
      if (opt.coerce) {
        element[`${opt.textNodeName}`]= coerce(text);
      } else {
        element[`${opt.textNodeName}`]= text;  
      }
    }
  ...
  xml.on('text', function(text) {
    curr.element[`${opt.textNodeName}`] = text;
    fa.run('state', [curr.element, curr.context, trace]);
    if (self._bufferLevel === 0 && self._emitData) {
      emitText.call(self, text);
    }
    if (!self._preserveAll) {
        var trimmed = curr.element[`${opt.textNodeName}`].trim();
        var spaced = curr.element[`${opt.textNodeName}`].substr(0, 1);
        spaced = (spaced !== '') && (spaced.trim() === '');
        var after = curr.element[`${opt.textNodeName}`].substr(-1, 1);
